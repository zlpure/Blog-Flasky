...
class APITestCase(unittest.TestCase):
    ...
    def test_bad_auth(self):
        ...
        u = User(email='john1@example.com',password='cat',confirmed=True,role=r)
        db.session.add(u)
        db.session.commit()

        response = self.client.post(url_for('api.get_posts'),headers=self.get_api_headers('john1@example.com','dog'))
        self.assertTrue(response.status_code == 401)

    def test_token_auth(self):
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u = User(email='john2@example.com', password='cat', confirmed=True, role=r)
        db.session.add(u)
        db.session.commit()
    ...
    def test_unconfirmed_account(self):
        ...
        self.assertTrue(response.status_code == 403 or 401 )
    ...
    def test_posts(self):
        ...
        self.assertTrue(response.status_code == 400 or 201 or 200)

        response = self.client.post(url_for('api.new_post'),
                                    headers=self.get_api_headers('john@example.com','cat'),
                                    data=json.dumps({'body':'body of the *blog* post'}))
        self.assertTrue(response.status_code == 201 or 200)
