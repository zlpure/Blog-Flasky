...
from .forms import ChangePassword,PasswordResetForm,PasswordResetRequestForm,ChangeEmailForm
from .. import db
...
@login_required
def secret():
    return 'Only authticated users are allowed!'
...
def password_reset_request():
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.email.data).first()
        if user:
            token = user.genetate_reset_token()
